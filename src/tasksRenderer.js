...
// Global Variables
const personalProjectsContainer = document.getElementById(
...
const displayProjectTasks = (projectName) => {
  const projectToDisplay = findProject(projectName);
  const projectTasks = projectToDisplay.taskList;
  tasksList.innerHTML = "";
...
const taskListeners = () => {
  ...
  formContainer.addEventListener("click", (e) => {
    if (e.target && e.target.id == "add-task-btn") {
      const activeProject = document.getElementById("project-title")
        .textContent;
